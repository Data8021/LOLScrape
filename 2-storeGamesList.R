...
## Strip all star game and worlds
...
minimizedList[[6]] <- scheduleList[[8]]
minimizedList[[7]] <- scheduleList[[9]]
...
for (i in 1:7) {
  ...
  for (k in 1:length(minimizedList[[i]][["highlanderTournaments"]][["id"]])) {
      ...
      ## Subset brackets
      bracketsList <- as.list(minimizedList[[i]][["highlanderTournaments"]][["brackets"]][[k]])

      ## Store bracketID
      bracketID <- bracketsList[[1]][k]
      
      ## extract matches
      matchesList <- as.list(bracketsList[["matches"]])
...
## Now go through everything again for All Stars and Worlds

## Strip all but all star and worlds
minimizedList <- as.list(0)
minimizedList[[1]] <- scheduleList[[1]]
minimizedList[[2]] <- scheduleList[[7]]
